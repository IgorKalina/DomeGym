...
class TestDeleteRoom:
    ...
    async def test_when_room_exist_should_return_200(
        self,
        ...
        # Act
        response, ok_response = await self._room_v1_api.delete(
            gym_id=room_v1.gym_id, subscription_id=room_v1.subscription_id, room_id=room_v1.id
        )

        # Assert
        assert response.status_code == HTTPStatus.OK
        assert len(ok_response.data) == 1
        assert len(ok_response.errors) == 0
        room: RoomResponse = ok_response.data[0]
        assert room == room_v1

    async def test_when_room_not_exists_should_return_404(self, gym_v1: GymResponse) -> None:
        # Arrange
        # is done via a fixture

        # Act
        response, response_data = await self._room_v1_api.delete(
            gym_id=gym_v1.id,
            subscription_id=gym_v1.subscription_id,
            room_id=constants.common.NON_EXISTING_ID,
        )
        ...
        assert error.title == "Room.Not_found"
        assert error.detail == f"Room with the provided id does not exist: {constants.common.NON_EXISTING_ID}"
