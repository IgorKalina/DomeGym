...
class TestGetGym:
    ...
    async def test_when_subscription_and_gym_exist_should_return_200(self) -> None:
        # Arrange
        # create subscription
        ...
        _, subscription_response = await self._subscription_v1_api.create(create_subscription_request)
        expected_subscription = subscription_response.data[0]
        # create gym
        create_gym_request = CreateGymRequest(name=constants.gym.NAME)
        _, gym_response = await self._gym_v1_api.create(
            request=create_gym_request, subscription_id=expected_subscription.id
        )
        expected_gym = gym_response.data[0]

        # Act
        response, response_data = await self._gym_v1_api.get(
            gym_id=expected_gym.id, subscription_id=expected_subscription.id
        )
    ...
    async def test_when_subscription_exists_and_no_gym_should_return_404(self) -> None:
        # Arrange
        create_subscription_request = CreateSubscriptionRequest(
            admin_id=constants.admin.ADMIN_ID,
            subscription_type=constants.subscription.DEFAULT_SUBSCRIPTION_TYPE,
        )
        await self._subscription_v1_api.create(create_subscription_request)
        subscription_response = await self._subscription_v1_api.get_subscription_by_admin_id(constants.admin.ADMIN_ID)
